...
class Sales_model extends Model
{
    ...
    protected $allowedFields = ['id', 'master_id', 'quantity', 'price', 'date', 'user_account', 'description'];
    ...
    public function export()
    {
        ...
        ->setDescription('Sales Data '.date("Y"))
        ->setKeywords('Sales')
        ->setCategory('Sales');

        // subtitle on row 3
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A3', 'Sales Data '.date("Y"));
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', "Sales Data");
        $spreadsheet->getActiveSheet()->getStyle('A1:'.$highestColumn.'1')->applyFromArray($this->headerStyle);

        $startRowBody = $startRowHeader+1;
        $index = 0;
        $data_sales = $this->orderBy($this->columnIndex, $this->order)->findAll();
        foreach ($data_sales as $key => $sales) {
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('A'.$startRowBody, ++$index);
            if(isset($sales->master_id))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('B'.$startRowBody, $sales->master_id);
            if(isset($sales->quantity))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('C'.$startRowBody, $sales->quantity);
            if(isset($sales->price))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('D'.$startRowBody, $sales->price);
            if(isset($sales->date))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('E'.$startRowBody, $sales->date);
            if(isset($sales->user_account))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('F'.$startRowBody, $sales->user_account);
            if(isset($sales->description))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('G'.$startRowBody, $sales->description);
            $startRowBody++;
        };

        $spreadsheet->getActiveSheet()->setTitle('Sales Data '.date('Y'));
        $spreadsheet->setActiveSheetIndex(0);
        // Redirect output to a client’s web browser (Xlsx)
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Sales '.date("Y").'.xlsx"');
